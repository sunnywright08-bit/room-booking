...
// One event per night, 19:00 -> 08:00 the following morning,
// repeating for the length of the stay. Times are Europe/Amsterdam.
...
export function buildIcs(booking) {
  ...
  // Each night: 19:00 on day N -> 08:00 on day N+1.
  // The series repeats once per night for the length of the stay.
  const start = localDateTime(checkIn, 19);
  const end = localDateTime(addDays(checkIn, 1), 8);
